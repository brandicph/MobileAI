...
def main(argv):
    ...
    y = np.sin(2*np.pi*np.random.rand(samples,1))
    ...
    for train_index, test_index in kf.split(X):
        ...
        print(X_test.shape)
        OSVR = OnlineSVR(numFeatures = X_test.shape[1], C = 10, eps = 0.1, kernelParam = 30, bias = 0, debug = debug)

        for i in range(X_test.shape[0]):
            print('%%%%%%%%%%%%%%% Data point {0} %%%%%%%%%%%%%%%'.format(i))
            OSVR.learn(X_train[i,:], y_train[i])


        print('X_test:',X_test)
        print('y_test:',y_test)

        y_predict = OSVR.predict(X_test)
        y_error = OSVR.computeMargin(X_test[0],y_test[0])
    ...
    #testSetY = np.array([[5.0],[3.0],[2.0],[1.0],[0.0]])
    testSetX = np.random.rand(50,2) * 1
    testSetY = np.random.rand(50,1) * 5
    #testSetX = np.random.rand(50,2)
    #testSetY = np.sin(2*np.pi*np.random.rand(50,1))
    ...
    #testX = np.random.rand(2,2)
    testX = np.random.rand(2,2) * 1
    testY = np.random.rand(2,1) * 5
    #testY = np.sin(2*np.pi*np.random.rand(2,1))
    print('testX:',testX)
    print('testY:',testY)
